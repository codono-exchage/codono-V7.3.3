...
class BuilderEdit extends AdminController
{
    ...
    public function keyTitle($name = 'title', $title = 'title', $subtitle = NULL)
    {
        return $this->keyText($name, $title, $subtitle);
    }

    public function keyId($name = 'id', $title = 'Numbering', $subtitle = NULL)
    {
    ...
    public function keyText($name, $title, $subtitle = NULL)
    {
	  ...
	  public function keyPrice($name, $title, $subtitle = NULL)
    {
        return $this->key($name, $title, $subtitle, 'text');
    }
    ...
    public function keyStatus($name = 'status', $title = 'Status', $subtitle = NULL, $map = NULL)
    {
        if (empty($map)) {
            $map = array('Disabled', 'Enabled');
        }
    ...
    public function keyAddTime($name = 'addtime', $title = 'add time', $subtitle = 'Default: Current Time')
    {
        return $this->keyTime($name, $title, $subtitle);
    }

    public function keyEndTime($name = 'endtime', $title = 'Edit time', $subtitle = 'Default: Current Time')
    {
        return $this->keyTime($name, $title, $subtitle);
    }
    ...
    {
        $map = array(1 => 'turn on', 0 => 'shut down');
        return $this->keyRadio($name, $title, $subtitle, $map);
    }
    ...
    public function handleConfig()
    {
        if (IS_POST) {
            ...
            if ($success) {
                ...
                exit(json_encode(array('info' => 'Save the configuration success!', 'status' => 1, 'url' => __SELF__)));
            } else {
                header('Content-type: application/json');
                exit(json_encode(array('info' => 'Save configuration failed!', 'status' => 0, 'url' => __SELF__)));
            }
        } else {
