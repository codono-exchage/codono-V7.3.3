...
class BuilderList extends AdminController
{
    ...
    private $_pagination = array('totalCount' => 0, 'listRows' => 0, 'parameter' => '');
    private $_data = array();
    ...
    public function key($name, $title, $type, $opt = NULL, $width = 'auto')
    {
    ...
    public function keyUserid($name = 'userid', $title = 'username')
    {
        return $this->key($name, $title, 'userid');
    }

    public function keyInvitid($name = 'invitid', $title = 'Recommended')
    {
        return $this->key($name, $title, 'userid');
    }

    public function keyShopid($name = 'shopid', $title = 'Product')
    {
	...
	public function keyPrice($name, $title)
    {
    ...
    public function keyStatus($name = 'status', $title = 'Status', $map = NULL)
    {
        if (empty($map)) {
            $map = array('Disabled', 'Enabled', 'Not Reviewed');
        }

        return $this->key($name, $title, 'status', $map);
    }

    public function keyType($name = 'type', $title = 'Type', $map = NULL)
    {
        return $this->key($name, $title, 'type', $map);
    }

    public function keyLink($name, $title = 'Option', $map = NULL)
    {
        if (is_array($map)) {
    ...
    public function select($title = 'select', $name = 'key', $type = 'select', $des = '', $attr, $arrdb = '', $arrvalue = NULL)
    {
        if (empty($arrdb)) {
    ...
    public function keyCreateTime($name = 'create_time', $title = 'Created')
    {
        return $this->keyTime($name, $title);
    }

    public function keyUpdateTime($name = 'update_time', $title = 'Updated')
    {
        return $this->keyTime($name, $title);
    }
    ...
    public function keyDoAction($getUrl, $text, $title = 'Option')
    {
        if (is_string($getUrl)) {
            $getUrl = $this->createDefaultGetUrlFunction($getUrl);
        }
    ...
    public function keyMycz($name = 'confirm', $title = 'Already paid', $getUrl)
    {
        if (is_string($getUrl)) {
            $getUrl = $this->createDefaultGetUrlFunction($getUrl);
        }

        return $this->key($name, $title, 'confirm', $getUrl);
    }

    public function keyDoActionRestore($text = 'reduction')
    {
        $that = $this;
    ...
    public function doSetStatus($model, $ids, $status = 1)
    {
        ...
        $this->success('Successful operation!', $_SERVER['HTTP_REFERER']);
    }
    ...
    public function display($templateFile = '', $charset = '', $contentType = '', $content = '', $prefix = '')
    {
        ...
        $this->convertKey('userid', 'text', function ($value) {
            ...
            if ($username) {
                return '<small><a href=\'' . U('User/index', array('field' => 'id', 'name' => $value)) . '\' target=\'_blank\'>' . $username . '</a></small>';
            } else {
                return '---';
            }
        });
        ...
        $this->convertKey('time', 'text', function ($value) {
            if ($value) {
                return '<small>'.addtime($value).'</small>';
            } else {
                return '---';
            }
        });
        $this->convertKey('status', 'html', function ($value, $key) {
             return '<small>'.$key['opt'][$value].'</small>';
        });
		...
		$this->convertKey('price', 'text', function ($value, $key, $item) {
            return NumToStr($value * 1);
        });
