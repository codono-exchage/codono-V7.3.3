...
class AirdropController extends CommonController
{
    public function index()
    {
        ...
        $featured= [];
        $is_featureds = $Model->where(['is_featured' => 1])->order('addtime desc')->limit(3)->select();
		...
		$page_title= L('Gift Rain');
        $data['page_title']=$page_title;
        $data['featured']=$featured;
        $data['list_upcoming']=$list_upcoming;
        $data['list_running']=$list_running;
        $data['list_ended']=$list_ended;
        $data['page']=$show;
        $this->ajaxShow($data);
    }
}
