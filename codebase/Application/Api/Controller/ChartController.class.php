...
class ChartController extends CommonController
{
    ...
    {
        $array = array('status' => 1, 'message' => 'connected to Chart API');
        echo json_encode($array);
    }

    public function getChart( )
    {
        $marketid=I('get.marketid',NULL,'intval');
        
        if (!$marketid) {
            $this->ajaxShow(array('marketid cannot be empty'), -1);
        }
		$marketid=I('marketid',0,'intval');
		$marketData=null;
    ...
    public function depth()
    {
        ...
        $market = $marketData['name'];
        $mo = M();
        $buy = $mo->query('select id,price,sum(num-deal)as nums from codono_trade where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit 100;');
        $sell = $mo->query('select id,price,sum(num-deal)as nums from codono_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit 100;');
