...
class ConvertController extends CommonController
{
    ...
    public function index()
    {
        ...
        $data['base_coin']= strtoupper($this->base_coin);
        $data['CoinList']=$CoinList;
        $data['reversedCoinList']=$reversedCoinList;
    ...
    {
        $from_coin = I('post.from_coin', '', 'htmlspecialchars');
        $to_coin = I('post.to_coin', '', 'htmlspecialchars');
    ...
    private function V2doTrade($from_coin, $to_coin, $amount, $type)
    {
        ...
        $command_path = PHP_PATH . " index.php /Api/Convert/callTrade/code/$json_input";
        exec($command_path);
    ...
    public function callTrade($code)
    {
        ...
        if (base64_encode(base64_decode($code, true)) === $code) {
            ...
            return A('Home/Trade')->upTrade($price, $amount, $type, 'limit', 0 , NULL, $market,$override );
        } else {
    ...
    {
        $market = strtolower($from_coin . '_' . $to_coin);
        return json_encode(A('Home/Ajax')->getActiveOrders($market, 1, false));
    }
