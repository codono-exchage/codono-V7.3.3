...
class Binance
{
    ...
    public function __construct()
    {
        ...
        switch (count($param)) {
            ...
            case 4:
                ...
                $this->api_secret = $param[1];
                $this->useTestnet = (bool)$param[2];
                $this->curlOpts = array('CURLOPT_SSL_VERIFYHOST' => FALSE, 'CURLOPT_SSL_VERIFYPEER' => FALSE);
                break;
            default:
     ...
     *
     * @param $member string the name of the property to return
     * @return null
     */
    ...
    protected function setupApiConfigFromFile(string $file = null)
    {
        ...
        $this->useTestnet = isset($contents['use-testnet']) && (bool)$contents['use-testnet'];
    }
     ...
     * @throws Exception
     */
    public function balances($priceData = array())
    {
    ...
    protected function httpRequest(string $url, string $method = "GET", array $params = [], bool $signed = false)
    {
        if (function_exists('curl_init') === false) {
            ...
            clog('binance',__FILE__.":Sorry cURL is not installed".__LINE__);
        }
    ...
    public function roundStep($qty, $stepSize = 0.1): float
    {
        ...
        return round(($qty / $stepSize) * $stepSize, $precision);
    }
     ...
     * });
     *
     * @param $symbols string optional array of symbols
    ...
    public function chart($symbols, string $interval = "30m", callable $callback = null, $limit = 500)
    {
        if (is_null($callback)) {
            ...
            clog('binance',__FILE__.":You must provide a valid callback at line".__LINE__);
            return false;
        }
        if (!is_array($symbols)) {
            $symbols = [
     ...
     * @throws Exception
     */
    public function accountSnapshot($type, $nbrDays = 5, $startTime = 0, $endTime = 0, $limit = 5)
    {
