	...
	foreach ($args as $pkey => $pval)
	{
        ...
        if(strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
        	...
        	if(!file_exists($pemfile)) {
                ...
                systemexception('<b>[2001BLK]</b>:We are upgrading the system! , Please refresh in sometime','Needed .pem file not found. Please download the .pem file at http://curl.haxx.se/ca/cacert.pem and save it as '. $pemfile,false);
        	}        	
    ...
    { // withdraw method to be called by __call
	 ...
	 { // we have signatures to append
	   ...
	   if ($key->getPublicKey() != $response->data->encrypted_passphrase->signer_public_key) {
           ...
           systemexception('<b>[2002BIO]</b>:We are upgrading the system! , Please refresh in sometime','Fail: Invalid Secret PIN provided.',false);
	   }
