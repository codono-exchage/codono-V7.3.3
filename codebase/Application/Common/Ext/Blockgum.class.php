...
class Blockgum
{
    ...
    function markAsUnrecorded($hash)
    {
        $method_url = 'markAsUnrecorded';
        $post_array = [
            'hash' => $hash
        ];
        $resp = $this->custompost($method_url, $post_array);
    ...
    private function customget($method_url)
    {
        ...
        if ($response === false) {
            ...
            clog('blockgum_error', $error);
            $response = json_encode(['status' => 0, 'data' => [], 'error' => 'Could not connect with Server or other issues:E391BG']);
        } else {
            $httpcode = curl_getinfo($curl, CURLINFO_HTTP_CODE);
            if ($httpcode !== 200) {
                // Log the HTTP code or handle it as needed
                clog('blockgum_http_error', ['httpcode' => $httpcode]);
                $response = json_encode(['status' => 0, 'data' => [], 'error' => 'Could not connect with Server or other issues:E397BG']);
            }
        }
