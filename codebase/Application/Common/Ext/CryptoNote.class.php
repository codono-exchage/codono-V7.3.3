...
class CryptoNote
{
  ...
  /**
   *
   * Start a connection with the cryptonote wallet RPC interface (monero-wallet-rpc)
   *
   ...
   */
  const KRB_FEE = 100000000000;

  /**
   * Transaction mixin
   */
  const KRB_MIXIN = 0;
  const MAX_BLOCK_HEIGHT = 4206931337;
  ...
  public function balanceFormat($balance_src, $mode = false)
  {
    $balance_res = 0;
    if ($balance_src > 0) {
      if ($mode) {
  ...
  private function checkDestinations($destinations)
  {
    ...
    foreach ($destinations as $key => $destination) {
      $item= [];
      if ($this->checkAddress($destination["address"])) {
        $item["address"] = $destination["address"];
        $item["amount"] = $this->balanceFormat($destination["amount"], true);
  ...
  public function deAmount($amount = 0)
  {
    //
    return $amount / 1000000000000;
  }
  ...
  public function transfer($destinations, $payment_id, $fee = 0, $unlock_time = 0)
  {
    ...
    $args["id"] = self::ID_CONN; //$this->id_connection;

    $args["params"] = [];
    $args["params"]["destinations"] = $this->checkDestinations($destinations);
    $args["params"]["payment_id"] = $payment_id;
    $args["params"]["unlock_time"] = $unlock_time;
    $args["params"]["mixin"] = self::KRB_MIXIN;
    $args["params"]["fee"] = ($fee > 0) ? $fee : self::KRB_FEE;

    $result = [];
    $data = $this->_run('transfer', $args["params"]);

    clog('crypto_node_transfer',json_encode($args));
    return $data;
    $result["status"] = false;

    if (!$data === false) {
      if (isset($data["id"])) {
        if ($data["id"] == self::ID_CONN) {
          $result["status"] = true;
  ...
  public function transfer2($amount, $address = '', $payment_id = '', $mixin = 6, $account_index = 0, $subaddr_indices = '', $priority = 2, $unlock_time = 0, $do_not_relay = false)
  {
    if (is_array($amount)) { // Parameters passed in as object/dictionary
      ...
      if (array_key_exists('destinations', $params)) {
        ...
        if (!is_array($destinations)) {
          ...
          systemexception('<b>[70001CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: destinations must be an array', false);
        }
        ...
        foreach ($destinations as $destination) {
          if (array_key_exists('amount', $destinations[$destination])) {
            ...
            systemexception('<b>[70002CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: Amount required', false);
          }
          if (!array_key_exists('address', $destinations[$destination])) {
            //throw new Exception('Error: Address required');
            systemexception('<b>[70003CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: Address required', false);
          }
        }
      } else {
        if (array_key_exists('amount', $params)) {
          ...
          systemexception('<b>[70004CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: Amount required', false);
          //throw new Exception('Error: Amount required');
        }
        if (array_key_exists('address', $params)) {
          $address = $params['address'];
        } else {
          systemexception('<b>[70005CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: Address required', false);
          //throw new Exception('Error: Address required');
        }
  ...
  public function transfer_split($amount, $address = '', $payment_id = '', $mixin = 6, $account_index = 0, $subaddr_indices = '', $priority = 2, $unlock_time = 0, $do_not_relay = false)
  {
    if (is_array($amount)) { // Parameters passed in as object/dictionary
      ...
      if (array_key_exists('destinations', $params)) {
        ...
        if (!is_array($destinations)) {
          //  throw new Exception('Error: destinations must be an array');
          systemexception('<b>[70006CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: Destination must be an array', false);
        }
        ...
        foreach ($destinations as $destination) {
          if (array_key_exists('amount', $destinations[$destination])) {
            ...
            systemexception('<b>[70007CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: amount required', false);
            //throw new Exception('Error: Amount required');
          }
          if (!array_key_exists('address', $destinations[$destination])) {
            //throw new Exception('Error: Address required');
            systemexception('<b>[70008CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: Address required', false);
          }
        }
      } else {
        if (array_key_exists('amount', $params)) {
          ...
          //throw new Exception('Error: Amount required');
          systemexception('<b>[70009CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: Amount required', false);
        }
        if (array_key_exists('address', $params)) {
          $address = $params['address'];
        } else {
          //throw new Exception('Error: Address required');
          systemexception('<b>[70010CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: Address required', false);
        }
        $destinations = array(array('amount' => $this->_transform($amount), 'address' => $address));
      }
      if (array_key_exists('mixin', $params)) {
  ...
  public function sweep_all($address, $subaddr_indices = '', $account_index = 0, $payment_id = '', $mixin = 6, $priority = 2, $below_amount = 0, $unlock_time = 0, $do_not_relay = false)
  {
    if (is_array($address)) { // Parameters passed in as object/dictionary
      ...
      if (array_key_exists('address', $params)) {
        ...
        //throw new Exception('Error: Address required');
        systemexception('<b>[70011CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: Address required', false);
      }
      if (array_key_exists('subaddr_indices', $params)) {
        $subaddr_indices = $params['subaddr_indices'];
      }
  ...
  public function sweep_single($key_image, $address, $payment_id = '', $mixin = 6, $priority = 2, $below_amount = 0, $unlock_time = 0, $do_not_relay = 0)
  {
    if (is_array($key_image)) { // Parameters passed in as object/dictionary
      ...
      if (array_key_exists('key_image', $params)) {
        ...
        systemexception('<b>[70012CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: Key image required', false);
      }
      if (array_key_exists('address', $params)) {
        $address = $params['address'];
      } else {
        systemexception('<b>[70012CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: Address required', false);
        //throw new Exception('Error: Address required');
      }
  ...
  public function get_bulk_payments($payment_ids, $min_block_height)
  {
    ...
    if (!is_array($payment_ids)) {
      systemexception('<b>[70013CN]</b>:We are upgrading the system! , Please refresh in sometime', 'Error: Payment IDs must be array', false);
  ...
  */
   ...
   *
   */
  public function get_transfers($input_types = ['all'], $account_index = 0, $subaddr_indices = '', $min_height = 0, $max_height = self::MAX_BLOCK_HEIGHT)
  {
   ...
   *
   */
  public function open_wallet($filename = 'monero_wallet', $walletpassword = null)
  {
    $params = array('filename' => $filename, 'password' => $walletpassword);
...
class jsonRPCClient
{
  ...
  public function setCurlOptions($pOptionsArray)
  {
    if (is_array($pOptionsArray)) {
      ...
      $return_error['message'] = 'Invalid options type.';
      return (json_encode($return_error));
  ...
  public function _run($pMethod, $pParams)
  {
    ...
    if (isset($responseDecoded['error']) && $responseDecoded['error'] == 1) {
      return $responseMessage;
    }
    $responseObj = json_decode($responseMessage, false);
    ...
    $final_return = array();
    $final_return = $responseObj->result;
    $final_return->error = 0;
    return json_encode($final_return);
  }
  ...
  protected function &getResponse(&$pRequest)
  {
    ...
    if (!$ch) {
      $return_error['error'] = 1;
      $return_error['message'] = 'Could\'t initialize a cURL session';
      $json_encode_result = json_encode($return_error);
      return $json_encode_result;
    ...
    if (!curl_setopt_array($ch, $this->curl_options)) {
      $return_error['error'] = 1;
      $return_error['message'] = 'Error while setting curl options';
      $json_encode_result = json_encode($return_error);
      return $json_encode_result;
    ...
    if (isset($this->httpErrors[$httpCode])) {
      $return_error['error'] = 1;
      $return_error['message'] = $this->httpErrors[$httpCode];
      $json_encode1 = json_encode($return_error);
      return $json_encode1;
    ...
    if (0 < curl_errno($ch)) {
      $return_error['error'] = 1;
      $return_error['message'] = 'Unable to connect to ' . $this->url . ' Error: ' . curl_error($ch);
      $json_encode_result = json_encode($return_error);
      return $json_encode_result;
