...
Sample TX:
  ...
  'StatusCode' => string '1' (length=1)
  'StatusMessage' => string '' (length=0)
  'TransactionStatus' => string 'PENDING' (length=7)
  'TransactionReference' => string 'qym7yrSZHqpwOlg9xjNKY03K0a2k7emtawJYrQ5TXlWgnM5b0uauvJUep7XiqMAJa0a01fa8282248f8bf78db8167524608' (length=96)

minimum is 500/= but I wanted to make mine to be 50,000
maximum is 7,000,000 which can still remain the same
Fees :3%

Sandbox
http://sandbox.yo.co.ug/services/yopaymentsdev/portal/
scottot
scottot
turndealer@gmail.com


API USER:90008360963
API PASS:1116694048
*/
class YoUganda {
    /**
     ...
     */
    public function __construct($username, $password,$mode='sandbox')
    {
		if($mode=='live'){
			$this->YOURL = "https://paymentsapi1.yo.co.ug/ybs/task.php";
		}else{
			$this->YOURL = 'https://sandbox.yo.co.ug/services/yopaymentsdev/task.php';
		}
    ...
    */
    ...
    public function generate_public_key_authentication_signature($msisdn, $amount, $narrative)
    {
        if($this->public_key_authentication_nonce==NULL){
            systemexception('Public key authentication nonce is not set. Please set it to continue');
        }

        if($this->private_key_file_location==NULL){
            systemexception('Private key file location cannot be NULL');
        }
        
        $fh = fopen($this->private_key_file_location, 'r');
        if($fh === FALSE) {
            systemexception('Private key file could not be opened. Confirm your file location '.$this->private_key_file_location);
        }
        ...
        if(is_bool($pkeyid)) {
            systemexception('Private key is invalid');
        ...
        if(method_exists('openssl', 'free_key')){
