...
class AuthGroupModel extends \Think\Model
{
    ...
    protected $_validate = array(
        'zend_ast' => array('un-handled kind  in zend_ast' => 'title', 'un-handled kind  in zend_ast' => 'require', 'un-handled kind  in zend_ast' => 'You must set a user group title', 'un-handled kind  in zend_ast' => \Think\Model::MUST_VALIDATE, 'un-handled kind  in zend_ast' => 'regex', 'un-handled kind  in zend_ast' => \Think\Model::MODEL_INSERT),
        'zend_ast2' => array('un-handled kind  in zend_ast' => 'description', 'un-handled kind  in zend_ast' => '0,80', 'un-handled kind  in zend_ast' => 'Description Up80character', 'un-handled kind  in zend_ast' => \Think\Model::VALUE_VALIDATE, 'un-handled kind  in zend_ast' => 'length', 'un-handled kind  in zend_ast' => \Think\Model::MODEL_BOTH)
    );
    ...
    public function checkId($modelname, $mid, $msg = 'With下iddoes not exist:')
    {
        if (is_array($mid)) {
            $count = count($mid);
            $ids = implode(',', $mid);
    ...
    {
        return $this->checkId('AuthGroup', $gid, 'The following user groupsiddoes not exist:');
    }

    public function checkCategoryId($cid)
    {
        return $this->checkId('Category', $cid, 'The following classificationiddoes not exist:');
    }
}