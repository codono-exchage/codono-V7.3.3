...
class ConfigModel extends \Think\Model
{
    ...
    public function construct()
    {
        ...
        if (!$this->daoqia) {
            $this->daoqia = $this->getUrl($this->authUrl . '/Auth/daoqi?mscode=' . CODONOLIC);
        ...
        if (!$this->shouhou) {
            $this->shouhou = $this->getUrl($this->authUrl . '/Auth/shouhou?mscode=' . CODONOLIC);
        ...
        if (!$this->game) {
            $this->game = $this->getUrl($this->authUrl . '/Auth/game?mscode=' . CODONOLIC);
    ...
    {
        dump('Authorizeareaname:' . $this->authUrl);
        dump('System expires:' . $this->daoqia);
        dump('Sale expires:' . $this->shouhou);
        dump('Authorized Application:' . $this->game);
    }
