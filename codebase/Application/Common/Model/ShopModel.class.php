...
class ShopModel extends \Think\Model
{
            ...
            if ($coin_arr) {
                ...
                foreach ($coin_arr as $k => $v) {
                    if (!in_array($k, $Shop_Coin_DbFields)) {
                        M()->execute('ALTER TABLE `codono_shop_coin` ADD COLUMN `' . $k . '` VARCHAR(50) NOT NULL  COMMENT \'' . $v . '\' AFTER `shopid`;');
                    }
                }
            }
                ...
                if (($k != 'id') && ($k != 'shopid')) {
                    if ($v) {
                        if ($k == 'usd') {
                            ...
                            $new_price = D('Market')->get_new_price($k . '_usd');
    ...
    public function setStatus($id = NULL, $type = NULL, $model = 'Shop')
    {
        if (empty($id)) {
            return null;
        }
