...
class TopupModel extends \Think\Model
{
    protected $key = 'Topup';

    public function get_type($name = NULL, $status = 1)
    {
        $list = M('TopupType')->where(array('status' => $status))->select();
    ...
    public function get_coin($name = NULL, $status = 1)
    {
        ...
        if (!$list) {
            $list = M('TopupCoin')->where(array('status' => $status))->select();
        ...
        /* 		foreach ($list as $k => $v) {
                    $price = (empty($v['price']) ? D('Market')->get_new_price($v['coinname'] . '_usd') : $v['price']);
    ...
    public function setStatus($id = NULL, $type = NULL, $model = 'Topup')
    {
        if (empty($id)) {
            return null;
        }
