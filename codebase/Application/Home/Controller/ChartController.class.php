...
class ChartController extends HomeController
{
    ...
    {
        $market = I('get.market', '', 'trim');
        $market = I('get.ajax', 'json', 'trim');
        ...
        if ($market) {
            ...
            if (!$codono_getCoreConfig) {
                ...
                $codono_ordinary = $codono_getCoreConfig['codono_userTradeDetailNum'];
                $codono_special = $codono_getCoreConfig['codono_specialUserTradeDetailNum'];
            }

            $limit = $codono_ordinary;
            if (userid()) {
                $usertype = $this->userinfo['usertype']; //M('User')->where(array('id' => userid()))->getField('usertype');
                if ($usertype == 1) {
                    $limit = $codono_special;
                } else {
                    $limit = $codono_ordinary;
                }
            }
    ...
    protected function getTradeBuy($market, $limit)
    {
        ...
        $mo = M();
        $buy = $mo->query('select id,price,sum(num-deal)as nums from codono_trade  where status=0 and type=1 and market =\'' . $market . '\' group by id order by price desc limit ' . $limit);
        $data = '';
        ...
        if ($buy) {
            ...
            foreach ($buy as $k => $v) {
                $data .= '<tr><td class=\'buy\'  width=\'50\'>Buy' . ($k + 1) . '</td><td class=\'buy\'  width=\'80\'>' . floatval($v['price']) . '</td><td class=\'buy\'  width=\'120\'>' . floatval($v['nums']) . '</td><td  width=\'100\'><span class=\'buySpan\' style=\'width: ' . (((min($maxNums, $v['nums'])) / $maxNums) * 100) . 'px;\' ></span></td></tr>';
            }
        }
    ...
    protected function getTradeSell($market, $limit)
    {
        $limit = intval($limit);
        $mo = M();
        $sell = $mo->query('select id,price,sum(num-deal)as nums from codono_trade where status=0 and type=2 and market =\'' . $market . '\' group by id order by price asc limit ' . $limit);
        $data = '';
        ...
        if ($sell) {
            ...
            foreach ($sell as $k => $v) {
                $data .= '<tr><td class=\'sell\'  width=\'50\'>Sale' . ($k + 1) . '</td><td class=\'sell\'  width=\'80\'>' . floatval($v['price']) . '</td><td class=\'sell\'  width=\'120\'>' . floatval($v['nums']) . '</td><td style=\'width: 100px;\'><span class=\'sellSpan\' style=\'width: ' . (((min($maxNums, $v['nums'])) / $maxNums) * 100) . 'px;\' ></span></td></tr>';
            }
        }
    ...
    public function getMarketNewKJson()
    {
        // TODO: SEPARATE
        $input = I('get.');
        $market = (is_array(C('market')[$input['market']]) ? trim($input['market']) : C('market_mr'));
