...
class DexController extends HomeController
{

    private $dexConfig;
    public const allowed_method = 'walletconnect';//  walletconnect or custom
    public function _initialize()
    {
        if (DEX_ALLOWED == 0) {
            $this->assign('type', 'Oops');
            $this->assign('error', 'Oops, Currently Dex is disabled!');
            //$this->display('Content/error');
            $this->error('Dex is currently disabled');
            exit();
        }
    ...
    {
        $default_buy_coin = '';
        $token_info = ['name' => $this->dexConfig['token_name'], 'symbol' => $this->dexConfig['token_symbol'], 'icon' => 'Upload/Coin/'.$this->dexConfig['token_image'], 'min' => $this->dexConfig['token_min'], 'max' => $this->dexConfig['token_max']];
        $buy_in_coins = $this->dexCoins();
    ...
    public function getPrice($spend_coin, $buy_qty = 0, $spend_qty = 0)
    {
        $expire = 300;
        ...
        if (!check($buy_qty, 'double') && !check($spend_qty, 'double')) {
            $this->error('Incorrect Quantity provided !' . $buy_qty);
        }
        if ($buy_qty > $spend_qty && $buy_qty > 0) {
            $type = 1;
            $amount = $buy_qty;
        } else {
            $type = 2;
        ...
        $expire_time = bcadd(time(), $expire);

        $resp = $this->getPriceOfCoin($spend_coin, $amount, $type);

        $quote_id = $this->genQid($uid);
        $saveme = ['qid' => $quote_id, 'userid' => $uid, 'spend_coin' => $spend_coin, 'buy_coin' => $this->dexConfig['token_symbol'], 'price' => $resp['price'], 'qty' => $resp['quantity'], 'total' => $resp['total'], 'addtime' => time(), 'expire' => (int)$expire_time];

        $mo = M();
        $if_saved = $mo->table('codono_dex_order')->add($saveme);

        if (isset($if_saved)) {
            $exit_data['status'] = 1;
            $exit_data['msg'] = 'You are being redirected';
            $exit_data['url'] = U('Dex/order', ['qid' => $quote_id]);
    ...
    private function getPriceOfCoin($spend_coin, $amount, $type): array
    {
        ...
        return ['spend_coin' => $spend_coin, 'quantity' => $quantity, 'price' => $price, 'total' => $total];
    }
    ...
    public function cronCoinDeposits()
    {
        ...
        if ($resp['count'] > 0) {
            ...
            if ($last_block_read > 0) {
                ...
                M('DexConfig')->where(['id' => 1])->save(['lastblock_coin' => $last_block_read]);
    ...
    public function cronTokenDeposits()
    {
        ...
        if ($resp['count'] > 0) {
            ...
            if ($last_block_read > 0) {
                $updated = M('DexConfig')->where(['id' => 1])->save(['lastblock_token' => $last_block_read]);
            }
    ...
    {
        $txs = M('DexTransactions')->where(['isdone' => 0, 'isError' => 0])->select();
        echo 'Found ' . count($txs) . ' abandoned transactions, Processing them<br/>';
        foreach ($txs as $tx) {
            echo 'abandonedDeposits starts<br/>';
            $record = M('DexDeposit');
            $if_found = $record->where(['in_hash' => $tx['hash']])->find();

            if (isset($if_found)) {
                //so this is already processed mark it as is done, so it doesn't pop up next time
                M('DexTransactions')->where(['hash' => $tx['hash']])->save(['isdone' => 1]);
            } else {
    ...
    {
        M('DexTransactions')->where(['id' => $id])->save(['isError' => 1]);
    }

    public function order($qid = 0)
    {
        if (!isset($qid) || $qid == 0) {
            redirect(U('Dex/index'));
        }
        $single_record = $this->findQID($qid);
        if ($qid == 0 || empty($single_record)) {
            redirect(U('Dex/index'));
        }
        $this->assign('record', $single_record);
		$this->assign('method', self::allowed_method);
        redirect(U('Dex/walletconnect/qid/'.$qid));
		if(self::allowed_method== 'custom'){
			$this->display('mm_order');
		}else{
			$this->display('wc_order');
		}
    ...
    public function walletconnect($qid)
    {
        ...
        if ($single_record['received'] == 1) {
            redirect(U('Dex/completed/qid/' . $qid));
        }
		...
		$this->assign('coin', $this->dexConfig['coin']);
		$this->assign('network', $this->dexConfig['network']);
    ...
    private function processAbandonedDeposits($qid, $transaction): void
    {
        ...
        if ($deposited_amount > 0) {
            ...
            if (isset($deposit_entry)) {
                M('DexDeposit')->where(['id' => $deposit_entry])->save(['payout_status' => 1]);
                ...
                if (stripos($payout_hash, '0x') === 0) {
                    $payment_update_array = ['payout_hash' => $payout_hash, 'payout_qty' => $payout_qty, 'payout_time' => time()];
                    M('DexDeposit')->where(['id' => $deposit_entry])->save($payment_update_array);
                }
            }
    ...
    private function processHash($qid, $hash): array
    {
        ...
        if ($configured_chain['chainid'] != $result->chainId) {
            return ['status' => 0, 'msg' => 'Incorrect chainid selected from Wallet', 'hash' => ''];
        }
		 if (strtolower($hash) != strtolower($result->hash)) {
            return ['status' => 0, 'msg' => 'Incorrect transaction hash', 'hash' => $hash];
        }
        ...
        if ($result->value === '0x0' && $result->input != '0x') {
            ...
            if (is_array($token_resp)) {
                ...
                return ['status' => 0, 'msg' => 'Please check txid for correct information:E35', 'hash' => ''];
        ...
        if (strtolower($to) != strtolower($this->dexConfig['receiver'])) {
            ...
            return ['status' => 0, 'msg' => 'Please check txid for correct information:E40', 'hash' => ''];

        }
		if ($receive_coin == null) {
            redirect(U('Dex/index?error=invalid_hash2'));
        }
        ...
        if ($deposited_amount > 0) {
            ...
            if (isset($deposit_entry)) {
                ...
                if (stripos($payout_hash, '0x') === 0) {
                    ...
                    $payment_update_array = ['payout_hash' => $payout_hash, 'payout_qty' => $payout_qty, 'payout_time' => time()];
                    M('DexDeposit')->where(['id' => $deposit_entry])->save($payment_update_array);
                    M('DexOrder')->where(['qid' => $qid])->save($payment_update_array);
                }
            }

        }
        return ['status' => 1, 'msg' => 'Please check txid for correct information:E40', 'hash' => $hash];
    }
    ...
    public function successPage()
    {
        ...
        if ($txhash['status'] == 1) {
            redirect(U('Dex/paid/hash/' . $txhash['hash']));
        }
        redirect(U('Dex/errorPage/msg' . $txhash['msg']));
    }
    ...
    private function processTokenDeposit($result)
    {
        ...
        if ($if_token_found) {
            return ['from' => $from, 'to' => $to, 'amount' => $amount, 'token' => $token_info['symbol']];
        } else {
            return false;
        }
    }
    ...
    private function depositEntry($entry)
    {
        ...
        if (!isset($hash_found)) {
            $deposit_Entry = ['userid' => $entry['userid'], 'qid' => $entry['qid'], 'in_hash' => $entry['in_hash'], 'in_address' => $entry['in_address'], 'coin' => $entry['coin'], 'amount' => $entry['amount'], 'in_time' => $entry['in_time']];
			
            $mo = M();
            $if_saved = $mo->table('codono_dex_deposit')->add($deposit_Entry);
            if ($entry['qid'] != 0 || $entry['qid'] != null) {
                $mo->table('codono_dex_order')->where(['qid' => $entry['qid']])->save(['received' => 1, 'connected_address' => $entry['in_address']]);
            }
