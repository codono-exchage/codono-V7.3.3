...
class IssueController extends HomeController
{
        ...
        foreach (json_decode($Issue['show_coin'], true) as $showco) {
            if ($showco) {
                if (!empty(C('coin')[$showco])) {
                    ...
                    $show_coins[] = ['id' => C('coin')[$showco]['id'], 'name' => C('coin')[$showco]['name'], 'price' => $the_price, 'balance' => $user_coin[$showco],'img'=>C('coin')[$showco]['img']];
                }
            }
        }
    ...
    */
        ...
        */
        ...
        try {
            ...
            if ($Issue['ownerid'] > 0 && $token_owners_keep>0 && $this->userExists($Issue['ownerid'])) {
                ...
                $rs['2'] = $mo->table('codono_invit')->add(['coin' => $payincoin, 'userid' => $Issue['ownerid'], 'invit' => userid(), 'name' => $conv_coin, 'type' => 'Issue of ICO Token:' . $num . $Issue['coinname'], 'num' => $token_owners_keep, 'mum' => $mum, 'fee' => $site_cut, 'addtime' => time(), 'status' => 1]);
            }
            ...
            if ($invit_num_1>0 && $User['invit_1'] && $Issue['invit_1'] && $this->userExists($User['invit_1'])) {
                    ...
                    $rs[] = $mo->table('codono_invit')->add(['coin' => $Issue['invit_coin'], 'userid' => $User['invit_1'], 'invit' => userid(), 'name' => $Issue['name'], 'type' => L('1st Tier ICO Bonus'), 'num' => $num, 'mum' => $mum, 'fee' => $invit_num_1, 'addtime' => time(), 'status' => 1]);
            }
            ...
            if ($invit_num_2>0 && $User['invit_2'] && $Issue['invit_2'] && $this->userExists($User['invit_2'])) {
                    ...
                    $rs['invit_num_2'] = $mo->table('codono_invit')->add(['coin' => $Issue['invit_coin'], 'userid' => $User['invit_2'], 'invit' => userid(), 'name' => $Issue['name'], 'type' => L('2bd Tier ICO Bonus'), 'num' => $num, 'mum' => $mum, 'fee' => $invit_num_2, 'addtime' => time(), 'status' => 1]);
            }
            ...
            if ($invit_num_3>0 && $User['invit_3'] && $Issue['invit_3']  && $this->userExists($User['invit_3'])) {
                    ...
                    $rs[] = $mo->table('codono_invit')->add(['coin' => $Issue['invit_coin'], 'userid' => $User['invit_3'], 'invit' => userid(), 'name' => $Issue['name'], 'type' => L('3rd Tir ICO Bonus'), 'num' => $num, 'mum' => $mum, 'fee' => $invit_num_3, 'addtime' => time(), 'status' => 1]);
    ...
    public function unlock($id)
    {
        ...
        $mo->startTrans();
        $rs = [];
        $rs[] = $mo->table('codono_user_coin')->where(['userid' => userid()])->setInc($IssueLog['coinname'], $jd_num);
