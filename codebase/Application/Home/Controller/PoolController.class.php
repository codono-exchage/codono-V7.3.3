...
class PoolController extends HomeController
{
    ...
    {
        $uid=userid();
        $count = M('Pool')->where(['status' => 1])->count();
        switch ($count) {
            case 1:
                $class = "col-md-12";
                break;
    ...
    */
    public function rentMachine()
    {
        ...
        $rs[] = $mo->table('codono_user_coin')->where(['userid' => $user['id']])->setDec($coinname, $total_required);


        $rs[] = $poollogid = $mo->table('codono_pool_log')->add(['poolid' => $pool['id'], 'userid' => $user['id'], 'coinname' => $pool['coinname'], 'getcoin' => $pool['getcoin'], 'name' => $pool['name'], 'ico' => $pool['ico'], 'price' => $pool['price'], 'num' => $input['num'], 'days' => $pool['days'], 'daily_profit' => $pool['daily_profit'], 'power' => $pool['power'], 'endtime' => time(), 'addtime' => time(), 'status' => 0, 'collected' => 0]);
        $rs[]=  $this->insertPoolFee($user['id'],$pool['id'], $poollogid, $total_required, $coinname, 1);
    ...
    */
    public function myMachines()
    {
        ...
        $show = $Page->show();
        $list = $PoolLog->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $pool_list = [];
        ...
        $this->assign('list', $pool_log_list);
    ...
    public function myRewards()
    {
        ...
        $Page = new Page($count, 15);
        $show = $Page->show();
        $list = $PoolRewards->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
