    ...
    */
    ...
    public function index()
    {
        ...
        if ($check['from'] < 0 || $check['to'] < 0 || ($from != 'spot' && $to != 'spot') || ($from == 'spot' && $to == 'spot')) {
            ...
            $from = 'spot';
            $to = $baltypes[1];
         // A('Transfer')->index($from,$to);
         redirect("/Transfer/index/from/$from/to/$to");
        }
        ...
        if ($from !== 'spot' && $to == 'spot') {
            ...
            $list = $this->userOtherbalances(userid(), $check['from']);
        }
    ...
    public function check()
    {
        ...
        $card_details = $mo->table('codono_giftcard')->where(['secret_code' => $secret_code])->find();
    ...
    private function doredeemCard($secret)
    {
        ...
        $mo = M();
        $secret_code = cryptString($secret);
        $card_details = $mo->table('codono_giftcard')->where(['secret_code' => $secret_code, 'status' => 1])->find();
        $cardid = $card_details['id'];
    ...
    public function doBuy()
    {
        ...
        $before_balance = $mo->table('codono_user_coin')->where(['userid' => userid()])->field([$coin, $coin . 'd'])->find();
        ...
        $gc_entry = ['owner_id' => userid(), 'coin' => $coin, 'card_img' => $bannerid, 'public_code' => $public_code, 'secret_code' => $secret_code, 'value' => $amount, 'nonce' => $nonce, 'addtime' => time(), 'status' => '1'];
