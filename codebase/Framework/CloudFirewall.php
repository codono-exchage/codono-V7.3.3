...
class CloudFirewall {
    ...
    public function changeSecurityLevel(string $value = 'low', string $zone = null) {
        if(self::checkSecurityLevel($value)) {
            if($zone) {
                return $this->connect('https://api.cloudflare.com/client/v4/zones/'.$zone.'/settings/security_level', 'PATCH', ['value' => $value]);
            } else {
                return $this->connect('https://api.cloudflare.com/client/v4/zones/'.$this->zone.'/settings/security_level', 'PATCH', ['value' => $value]);
            }
        } else {
            return false;
        }
    ...
    public function createAccessRule(string $value, string $action) {
        if(self::checkIP($value) && self::checkAccessRule($action)) {
            return $this->connect('https://api.cloudflare.com/client/v4/user/firewall/access_rules/rules', 'POST', ['mode' => $action, 'configuration' => ['target' => (self::checkIPv4($value) ? 'ip' : (static::checkIPv4($value) ? 'ip6' : null)), 'value' => $value], 'notes' => 'Created by CloudFirewall']);
        } else {
            return false;
        }
    }
    ...
    private function sqlCheck(string $value, string $method, string $displayName, bool $ban) {
        // Replace common contractions with their full forms to prevent false positives
        $replace = ["can't" => 'can not', "don't" => 'do not'];
        foreach ($replace as $key => $value_rep) {
            $value = str_replace($key, $value_rep, $value);
        }
    
        // Extract potential SQL-sensitive parts
        $valueParts = explode('/', $value);
        
        foreach ($valueParts as $part) {
            // Whitelist Ethereum addresses to avoid false positives
           if (preg_match('/^0x[a-fA-F0-9]{40}$/', $part) || preg_match('/^0x[a-fA-F0-9]{64}$/', $part)) {
			continue; // Skip Ethereum or longer addresses
		   }
    
            // Get the list of potentially harmful SQL keywords and patterns
            $badWords = self::getVulnTypeData('SQL');
            
            foreach ($badWords as $badWord) {
                // Check for bad words while ignoring case sensitivity
                if (strpos(strtolower($part), strtolower($badWord)) !== false) {
                    if ($ban) {
                        $this->createAccessRule($this->getIP(), 'block'); // Block the IP if ban is enabled
                    }
                    self::abort(403, 'SQL Injection Detected'); // Abort with an error message
                }
            }
        }
    }
    ...
    protected function getIP() {
        $SIP=$_SERVER['REMOTE_ADDR']??0;
        return ($_SERVER['HTTP_CF_CONNECTING_IP'] ?? $SIP);
    }
    ...
    protected function connect(string $url, string $request, array $fields) {
        ...
        curl_setopt($this->curl, CURLOPT_HTTPHEADER, ['X-Auth-Email: '.$this->email, 'X-Auth-Key: '.$this->key, 'Content-Type: application/json']);
        return curl_exec($this->curl);
    }

    protected static function checkSecurityLevel(string $value) {
        return in_array($value, ['essentially_off', 'low', 'medium', 'high', 'under_attack']);
    }
    protected static function checkAccessRule(string $value) {
        return in_array($value, ['block', 'challenge', 'whitelist', 'js_challenge']);
    }
    ...
    protected static function checkIP(string $value) {
        return filter_var($value, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) || filter_var($value, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6);
    }
