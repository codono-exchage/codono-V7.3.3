...
class AdvModel extends Model
{
    ...
    public function setLazyInc($field, $step = 1, $lazyTime = 0)
    {
        ...
        if (empty($condition)) { // NoconditionWithout anyUpdate
            return false;
        }
        if ($lazyTime > 0) {// delayWrite
            $guid = md5($this->name . '_' . $field . '_' . serialize($condition));
            $step = $this->lazyWrite($guid, $step, $lazyTime);
            if (false === $step) return true; // Wait for the nextWrite
        }
        return $this->setField($field, array('exp', $field . '+' . $step));
    }

    /**
     * Field values reduce latency
     * @access public
     * @param string $field Field Name
    ...
    {
        if (false !== ($value = F($guid))) { // existCachedata input
