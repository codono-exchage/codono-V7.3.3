...
/**
 * willTraceInformation to thechromeBrowseThe deviceController, Which does not affectajaxAnd effectpageoflayout。
 * usebefore,youneedfirstinstallation chrome log This plug-in: http://craig.is/writing/chrome-logger
 * Defined applicationstags.phpfile Application/Common/Conf/tags.php,
 * <code>
 * <?php return array(
 *   'app_end'=>array(
 ...
 * );
 * </code>
 * in casetraceinformationNonormalExport,Please check yourofJournal。
 * This is done byhttp headerswithchromecommunication,So to ensure thatExporttraceCan not have prior information
 * headersExport,youallowableEntrancefilefirst rowJoinCode ob_start(); Or configureoutput_buffering
 *
 */
...
class ChromeShowPageTraceBehavior
{
    ...
    protected $tracePageTabs = array('BASE' => 'Basic', 'FILE' => 'file', 'INFO' => 'Process', 'ERR|NOTIC' => 'error', 'SQL' => 'SQL', 'DEBUG' => 'Debug');
    ...
    private function showTrace()
    {
        ...
        chrome_debug('TRACE information:' . __SELF__, 'group');
        //Output Log
        foreach ($trace as $title => $log) {
            'error' == $title ? chrome_debug($title, 'group') : chrome_debug($title, 'groupCollapsed');
...
//ChromePhp ExporttraceThe function
    function chrome_debug($msg, $type = 'trace', $trace_level = 1)
    {
        if ('trace' == $type) {
            ...
            if (method_exists('Behavior\ChromePhp', $type)) {
                //stand bytype trace,warn,log,error,group, groupCollapsed, groupEndWait
                call_user_func(array('Behavior\ChromePhp', $type), $msg);
            } else {
                //in casetypeIt is nottrace,warn,logAnd so on, for thelogs Mark
    ...
    class ChromePhp
    {
        ...
        {
            return base64_encode(mb_convert_encoding(json_encode($data), 'UTF-8', 'UTF-8'));
        }
