...
abstract class Driver
{
    ...
    // Name of the model to which the current operation belongs
    ...
    // Returns or affects the number of records
    protected $numRows = 0;
    // Things operation PDO instance
    protected $transPDO = null;
    // Number of transaction instructions
    protected $transTimes = 0;
    // Error message
    protected $error = '';
    // Database connection ID supports multiple connections
    ...
    protected $config = array(
        ...
        'hostport'       => '', // SQL Port
        'dsn'            => '', //
        'params'         => array(), // Database connection parameters
        'charset'        => 'utf8', // Database encoding uses utf8 by default
        'prefix'         => '', // Database table prefix
        'debug'          => false, // Database debug mode
        'deploy'         => 0, // Database deployment method: 0 centralized (single server), 1 distributed (master-slave server)
        'rw_separate'    => false, // Whether the database read and write are separated
        'master_num'     => 1, // Number of master servers after read and write separation
        'slave_no'       => '', // Specify the slave server number
        'db_like_fields' => '',
    );
    // Database expression
    ...
    /**
     * Schema function Read database configuration information
     * @access public
        ...
        if (!isset($this->linkID[$linkNum])) {
            ...
            try {
                ...
                if (version_compare(PHP_VERSION, '5.3.6', '<=')) {
                    // Disable mock prepared statements
				...
				systemexception('<b>[10001]</b>:We are upgrading the system! , Please refresh in sometime',$e->getMessage(),false);
                if ($autoConnection) {
    ...
    /**
     * Parse dsn information for pdo connection
     * @access public
    ...
    /**
     * Execute query return data set
     * @access public
     * @param string $str  sql command
     * @param boolean $fetchSql  Do not execute just get SQL
     * @param boolean $master  Whether to read on the master server
     * @return mixed
     */
    ...
    public function error()
    {
        ...
        if ($this->config['debug']) {
            // Turn on database debugging mode
            E($this->error);
        } else {
            return $this->error;
        }
