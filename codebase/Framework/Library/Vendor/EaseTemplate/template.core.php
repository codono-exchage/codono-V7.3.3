...
class ETCore
{
    ...
    )
    {
        ...
        //Detect runmethod
        $run = 0;
        if (preg_match("/run:/i", $ShowTPL)) {
            $run = 1;
            // Fix =
            $ShowTPL = preg_replace('/({|<!--\s*)run:\s*=\s*/i', '{run:echo ', $ShowTPL);
            // Fix Run 1
            $ShowTPL = preg_replace('/({|<!--\s*)run:\s*(.*?)\s*\/run(\}|<!--\s*)/is', '(T_T)\\2;(T_T!)', $ShowTPL);
        }
        // Fix XML
        if (stripos($ShowTPL, '<?xml') !== false) {
