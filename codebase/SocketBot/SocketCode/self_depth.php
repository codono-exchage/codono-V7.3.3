...
$worker = new Worker('websocket://0.0.0.0:7273',$context);
$worker->onWorkerStart = function() {
    global $redis;
    $redis = new Client('redis://127.0.0.1:6379');
		$redis->auth(REDIS_PASSWORD, function ($result) use ($redis) {
			if ($result !== true) {
...
$worker->onMessage = function($connection,$data) {
    ...
    Timer::add($time_interval, function () use ($connection, $args) {
        ...
        $redis->get($depth_key, function ($result) use ($market, $connection) {
			...
			if($sign1>0){
			$r1=bcadd(1,bcdiv(rand(1,99),100000,8),8);
			}else{
			$r1=bcadd(1,bcdiv(rand(1,99),1000000,8),8);
			}
			
			if($sign2>0){
			$r2=bcadd(1,bcdiv(rand(1,99),100000,8),8);
			}else{
			$r2=bcadd(1,bcdiv(rand(1,99),1000000,8),8);
			}
			
			if($sign3>0){
			$r3=bcsub(1,bcdiv(rand(1,99),100000,8),8);
			}else{
			$r3=bcsub(1,bcdiv(rand(1,99),1000000,8),8);
			}
			
			if($sign4>0){
			$r4=bcsub(1,bcdiv(rand(1,99),100000,8),8);
			}else{
			$r4=bcsub(1,bcdiv(rand(1,99),1000000,8),8);
			}
			...
			$xcross=array_merge($info[$market][1]['depth']['buy'],$info[$market][1]['depth']['sell']);
